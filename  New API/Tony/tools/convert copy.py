...
def yaml_to_json(yaml_file, json_schema, json_file):
    try:
        ...
        result = {}
        set_default_values(result, schema)  # set default values for missing properties
        for key, value in schema["properties"].items():
            if key in yaml_content:
                if value["type"] == "object":
                    result[key] = {}
                    set_default_values(result[key], value)  # set default values for nested properties
                    for nested_key, nested_value in value["properties"].items():
                        if nested_key in yaml_content[key]:
                            result[key][nested_key] = yaml_content[key][nested_key]
                        elif "default" in nested_value:
                            result[key][nested_key] = nested_value["default"]
                else:
